...
class Binary_Loss:
    def calculate(self, targets, predictions):
        predictions = np.clip(predictions, 1e-7,
                              1 - 1e-7)  # si regola min e max tale che min-->o e max-->1 ma non ci arrivi mai
...
def normalize_data(X):
    ...
    for i in range(n_columns):
        # getting the "i-esima" column
        X_col = X[:, i]
        # normalizing all the values in the column
        X_col_norm = (X_col - X_col.min()) / (X_col.max() - X_col.min())
        # replacing the original column with the normalized one
        X[:, i] = X_col_norm
...
def denormalize_data(X_normalized, X_original):
    # getting number of columns
    n_columns = len(X_normalized[0, :])
    i = 0
    for i in range(n_columns):
        # getting the "i-esima" column
        X_col = X_normalized[:, i]
        X_col_original = X_original[:, i]
        # de-normalizing all the values in the column
        X_col_denormalized = X_col * (X_col_original.max() - X_col_original.min()) + X_col_original.min()
        # replacing the normalized column with the de-normalized one
        X_normalized[:, i] = X_col_denormalized
        ++i
...
ax.legend([g_train_loss, g_train_accuracy, g_valid_loss, g_valid_accuracy],
